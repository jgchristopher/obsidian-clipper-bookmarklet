class MarkdownTablesHelper {
  ...
  static isFirstTbody(element: (HTMLElement | ParentNode)): boolean {
    ...
    if (previousSibling) {
      if (element.nodeName !== 'TBODY') { // element isn't a tbody
        isFirstTbody = false;
      } else {
        if (!previousSibling) {
          isFirstTbody = true;
  ...
  static cell(content: string, node: (HTMLElement | null) = null, index: (number | null) = null) {
    ...
    if (node) filteredContent = MarkdownTablesHelper.handleColSpan(filteredContent, node, ' ');
    ...
    let rules = {
      tableCell: {
        ...
        replacement: function(content: string, node: HTMLTableElement) {
          if (MarkdownTablesHelper.tableShouldBeSkipped(MarkdownTablesHelper.nodeParentTable(node))) return content;
      ...
      tableRow: {
        ...
        replacement: function(content: any, node: any) {
          ...
          if (MarkdownTablesHelper.tableShouldBeSkipped(parentTable)) return content;

          let borderCells = ''
          let alignMap: any = { left: ':--', right: '--:', center: ':-:' }
          ...
          if (MarkdownTablesHelper.isHeadingRow(node)) {
            ...
            for (let i = 0; i < colCount; i++) {
              ...
              let border: any = '---'
              let align = childNode ? (childNode.getAttribute('align') || '').toLowerCase() : '';
      ...
      table: {
        ...
        replacement: function(content: any, node: any) {
          ...
          let columnCount = MarkdownTablesHelper.tableColCount(node);
          let emptyHeader = ''
