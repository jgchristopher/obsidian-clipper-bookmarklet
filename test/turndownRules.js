...
function escapePipeCharacters(content) {
  return content.replace(/\|+/g, '\\|'); // escape any | characters in the content
}

function replaceWhitespace(content) {
...
function handleTH(rootNode) {
  ...
  turndownService.addRule('th', {
    ...
    replacement: function(content, node, options) {
      headerCount++
      content = escapePipeCharacters(content);
      content = content + " | ";
      return content;
    }
  });
  let md = turndownService.turndown(rootNode)
  return replaceWhitespace(md);
}
